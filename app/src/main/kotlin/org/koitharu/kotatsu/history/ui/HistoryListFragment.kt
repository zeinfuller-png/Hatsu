...
class HistoryListFragment : MangaListFragment() {
	...
	override fun onEmptyActionClick() {
		val emptyState = viewModel.content.value?.filterIsInstance<EmptyState>()?.firstOrNull()
		if (emptyState?.actionNavId != 0 && emptyState?.actionNavId != null) {
			(requireActivity() as? BottomNavOwner)?.bottomNav?.selectedItemId = emptyState.actionNavId
		} else {
